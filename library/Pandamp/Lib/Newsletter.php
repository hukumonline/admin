...
class Pandamp_Lib_Newsletter{
     ...
     * Constructor      
     */
    function Pandamp_Lib_Newsletter(){  
    ...
    function addMail($params){
    	...
    	if ( $error===true ){
			...
			$tblQueue = new App_Model_Db_Table_Queue();
		...
		if ( count($mails)>0 ){
			...
			foreach ( $mails as $mail ){  
				...
				//else {
						...
						$this->MailObj->Host 		= $config->mail->host;
						$this->MailObj->SMTPAuth 	= $config->mail->auth;
						$this->MailObj->Username	= $config->mail->username;
						$this->MailObj->Password	= $config->mail->password;
						$this->MailObj->From		= $config->mail->sender->support->email;
						$this->MailObj->FromName	= $config->mail->sender->support->name; 
						
						$this->MailObj->AddAddress($mail->recepientMail, $mail->recepientName);
						$this->MailObj->AddReplyTo($config->mail->sender->support->email, $config->mail->sender->support->name);
